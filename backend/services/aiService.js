 ...
 */
const generateDailySummary = async (user_id) => {
  ...
  // const recentWorkouts = await WorkoutSession.findAll({ where: { user_id }, limit: 7, order: [['startTime', 'DESC']] });
  ...
  const suggestionPromises = suggestions.map(s => AISuggestion.create({ ...s, user_id }));
 ...
 */
const createWorkoutPlan = async (user_id, options = {}) => {
 ...
 */
const createMealPlan = async (user_id) => {
